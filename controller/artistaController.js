...
exports.register = async (req, res) => {
  ...
  exports.verifyCode = (req, res) => {
    ...
    // Consulta para verificar si el código y el correo coinciden
    const query = "SELECT * FROM usuarios WHERE email = ? AND verification_code = ? LIMIT 1";
    ...
    connection.query(query, [email, code], (err, results) => {
      ...
      // Si no se encuentra el correo o el código es incorrecto
      if (results.length === 0) {
        return res.status(400).json({ message: "Código incorrecto o expirado." });
      }
  
      // Si el código es correcto, actualizamos el usuario para marcarlo como verificado
      const updateQuery = "UPDATE usuarios SET verification_code = NULL WHERE email = ? AND verification_code = ?";
      ...
      connection.query(updateQuery, [email, code], (updateErr) => {
        ...
        // Responder que la verificación fue exitosa
  ...
  exports.login = (req, res) => {
    ...
    connection.query(query, [email], async (err, results) => {
      ...
      // Generar el token JWT si el usuario está verificado
...
exports.uploadMusic = (req, res) => {
  ...
  const letra_explicita =
    req.body.letra_explicita && req.body.letra_explicita.toLowerCase() === "si"
      ? 1
      : 0;
  const es_cover =
    req.body.es_cover && req.body.es_cover.toLowerCase() === "si" ? 1 : 0;
  const publicado_antes =
    req.body.publicado_antes && req.body.publicado_antes.toLowerCase() === "si"
      ? 1
      : 0;
  ...
  if (!fecha_lanzamiento || !/^\d{4}-\d{2}-\d{2}$/.test(fecha_lanzamiento)) {
    return res
      .status(400)
      .json({ message: "Fecha de lanzamiento inválida o no proporcionada" });
  }
  // Convertir "si" a 1, y cualquier otro valor a 0
  ...
  connection.query(query, values, (err, results) => {
    ...
    res.status(201).json({
      message: "Lanzamiento subido correctamente",
      id: results.insertId,
    });
  });
};
